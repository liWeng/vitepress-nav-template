...
mkdir -p /data/software
cd /data/software
wget http://www.mgf.show/static/download/software/Percona-XtraDB-Cluster_8.0.23-14.1_Linux.x86_64.glibc2.17.tar.gz
tar zxvf Percona-XtraDB-Cluster_8.0.23-14.1_Linux.x86_64.glibc2.17.tar.gz
mv Percona-XtraDB-Cluster_8.0.23-14.1_Linux.x86_64.glibc2.17 /usr/local/mysql/pxc8.0.23
```
...
```
export PATH=$PATH:/usr/local/mysql/pxc8.0.23/bin
```
...
```
wget http://yth.365grid.cn/static/download/pxc5.7/mysql.server.sh -O /usr/local/mysql/pxc8.0.23/mysql.server.sh
chmod 755 /usr/local/mysql/pxc8.0.23/mysql.server.sh
```
...
basedir=/usr/local/mysql/pxc8.0.23
...
```
bin/mysqld --defaults-file=/data/mysql/30365/etc/my.cnf --initialize  
```

```
./mysql.server.sh --defaults-file=/data/mysql/30365/etc/my.cnf start
```
...
```
 登陆节点
mysql -u root -h 127.0.0.1 -P30365 -p";5&TyDuwqLUF"
```
```
修改密码、创建用户

ALTER  USER  'root'@'localhost'  IDENTIFIED  BY  'Hhwy@admin2021' ;

CREATE USER admin IDENTIFIED BY 'Hhwy@admin2021';
GRANT ALL PRIVILEGES ON *.* TO 'admin'@'%' ;

CREATE USER sstuser IDENTIFIED BY 'Hhwy@sst2021';
