...
cd /data/software
wget http://monitor.mgf.show/static/download/software/percona-xtrabackup-24-2.4.15-1.el7.x86_64.rpm
...
```
wget http://monitor.mgf.show/static/download/shell/mysql.server.sh -O /usr/local/mysql/pxc5.7.27/mysql.server.sh
...
```
修改密码、创建用户
alter user user() identified by 'Hhsy@admin2023';
GRANT ALL PRIVILEGES ON *.* TO 'admin'@'%' IDENTIFIED BY 'Hhsy@admin2023' WITH GRANT OPTION; 
GRANT ALL PRIVILEGES ON *.* TO 'admin'@'localhost' IDENTIFIED BY 'Hhsy@admin2023' WITH GRANT OPTION; 
GRANT ALL PRIVILEGES ON *.* TO 'sstuser'@'%' IDENTIFIED BY 'Hhsy@sst2023' WITH GRANT OPTION; 
GRANT ALL PRIVILEGES ON *.* TO 'sstuser'@'localhost' IDENTIFIED BY 'Hhsy@sst2023' WITH GRANT OPTION; 
flush privileges;
```
